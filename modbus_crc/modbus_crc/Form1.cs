using System.Diagnostics;

namespace modbus_crc
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            InitializeComponent();
        }

        private static readonly byte[] aCrcHi = new byte[256]
            {
                
            };
        private static readonly byte[] aCrcLo = new byte[256]
        {
                
        };

        private void BtnCalculateCrc_Click(object sender, EventArgs e)
        {
            string inputHex = TxtBytesInput.Text.Trim();
            string[] hexBytes = inputHex.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            byte[] data;
            try
            {
                data = hexBytes.Select(b => Convert.ToByte(b, 16)).ToArray();
            }
            catch
            {
                MessageBox.Show("Nieprawid³owy format bajtów. Upewnij siê, ¿e wszystkie s¹ w formacie hex (np. 0A, FF).", "B³¹d danych");
                return;
            }

            if (!long.TryParse(TxtInputIterations.Text.Trim(), out long iterations) || iterations < 1 || iterations > 1_000_000_000)
            {
                MessageBox.Show("Podaj poprawn¹ liczbê iteracji (1..1 000 000 000).", "B³¹d danych");
                return;
            }

            Stopwatch sw = Stopwatch.StartNew();
            ushort crc = 0;
            for (long i = 0; i < iterations; i++)
            {
                crc = CalculateCRC(data);
            }
            sw.Stop();

            TxtOutputCrc.Text = crc.ToString("X4");
            TimeSpan duration = sw.Elapsed;
            double totalMs = duration.TotalMilliseconds;
            double avgMs = totalMs / iterations;
            TxtSumTime.Text = FormatTime(totalMs);
            TxtAverageTime.Text = FormatTime(avgMs);
        }

        private static UInt16 CalculateCRC(byte[] data)
        {
            byte hi = 0xFF;
            byte lo = 0xFF;
            for (int i = 0; i < data.Length; i++)
            {
                int index = hi ^ data[i];
                hi = (byte)(lo ^ aCrcHi[index]);
                lo = aCrcLo[index];
            }
            return (ushort)((hi << 8) | lo);
        }

        private string FormatTime(double timeMs)
        {
            if (timeMs >= 1)
                return $"{timeMs:N3} ms";
            else if (timeMs >= 0.001)
                return $"{timeMs * 1000:N3} µs";
            else
                return $"{timeMs * 1_000_000:N3} ns";
        }
    }
}